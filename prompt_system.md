...
1. **RÈGLE COMPILATION & DÉPLOIEMENT MANDATOIRE :** Il est absolument obligatoire de reconstruire et redémarrer l'application avec `./deploy.sh` dès que du code Rust (`src/**/*.rs` ou `Cargo.toml`) est modifié. L'utilisation de `./deploy.sh` permet d'appeler l'API locale du bot `/api/maintenance` pour poster instantanément un message d'avertissement de mise à jour sur le chat Twitch dès le début de la compilation Docker. Tu dois exécuter et valider un `cargo check` ou un build via `./deploy.sh` réussi avant de valider ton code auprès de l'utilisateur.
